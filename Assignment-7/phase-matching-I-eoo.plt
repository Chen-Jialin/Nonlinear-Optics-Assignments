...
set xtics ('$0$' 0, '$0.3608$' 0.36077978751849696844481691830015, '$0.6$' 0.6, '$0.8$' 0.8, '$1.0$' 1.0, '$1.2$' 1.2, '$1.4$' 1.4, '$\pi/2$' pi/2)
set arrow nohead from 0,0 to pi / 2,0 lw 2 lc rgb 'black'
set arrow nohead from 0.36077978751849696844481691830015,-2.5e14 to 0.36077978751849696844481691830015,5e13 dt 2 lw 2 lc rgb 'gray'
set output 'phase-matching-I-eoo.tex'
plot 1 / sqrt((cos(x) / n_O(omega_p))**2 + (sin(x) / n_E(omega_p))**2) * omega_p - n_O(omega_s) * omega_s - n_O(omega_i) * omega_i
set output